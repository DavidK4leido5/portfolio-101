 ...
 */
const readHeroMark = (page) => page.evaluate(() => {
  ...
  return {
    ...
    sizes: strokes.map((el) => Math.round(parseFloat(el.style.fontSize || '0'))),
    ...
    hasStrokeNoFill: strokes.every((el) => getComputedStyle(el).fill === 'none'),
...
const traceOffset = (page, progress) => page.evaluate((p) => {
...
const stageOpen = (page, index) => page.waitForFunction(
  (i) => {
...
async function backToHero(page) {
  ...
  await page.waitForSelector('.ui[data-load-phase="ready"][data-scroll-zone="hero"]', {
    timeout: T.settle,
  })
}
...
await desktop.waitForSelector('.ui[data-scroll-zone="hero"]', { timeout: T.settle })
...
else if (framing.uiTier !== 'desktop') fail(`desktop UI: data-quality-tier=${framing.uiTier}`)
else if (Math.abs(framing.camZ - 9.8) > 0.15) fail(`desktop framing: camZ=${framing.camZ}, expected ~9.8`)
else if (Math.abs(framing.clusterScale - 1.18) > 0.02) fail(`desktop framing: scale=${framing.clusterScale}`)
...
if (mark.missing) fail('desktop: hero mark missing')
  ...
  if (mark.knockoutWords.join('') !== 'FULLSTACK') {
    fail(`hero mark: knockout words were ${JSON.stringify(mark.knockoutWords)}, expected FULL + STACK`)
  } else console.log('ok: FULL / STACK punched out of the veil in two lines')

  if (!mark.veilMask?.includes('hero-knockout')) {
    fail('hero mark: veil is not masked by the glyphs, so nothing is cut out of it')
  } else console.log('ok: veil carries the glyph mask')

  if (!mark.hasStrokeNoFill) {
    fail('hero mark: outline copy should be stroke-only — a filled one hides the cloud it frames')
  ...
  if (spread > measure * 0.015) {
    fail(`hero mark: lines not fitted to one measure, widths ${JSON.stringify(mark.widths)}`)
  } else console.log(`ok: both lines fitted to the same measure (~${measure}px, ${spread}px apart)`)

  if (mark.sizes[0] <= mark.sizes[1]) {
    fail(`hero mark: FULL should be set larger than STACK to fill the same measure, got ${JSON.stringify(mark.sizes)}`)
  } else console.log(`ok: per-line sizes solved (${mark.sizes.join(' / ')}px)`)

  if (mark.inkLeft < 0 || mark.inkRight > mark.vw) {
    fail(`hero mark: block overhangs the viewport (${mark.inkLeft}..${mark.inkRight} of ${mark.vw})`)
  } else if (mark.inkTop < 0 || mark.inkBottom > mark.vh) {
    fail(`hero mark: block clipped vertically (${mark.inkTop}..${mark.inkBottom} of ${mark.vh})`)
...
} else console.log(`ok: brain touch raycast active (strength=${touchOn.strength.toFixed(2)})`)
...
for (const at of [0, 0.3, 0.6, 1]) {
  await desktop.evaluate((f) => window.scrollTo(0, window.innerHeight * f * 0.98), at)
  await desktop.waitForTimeout(1400)
  zoomSamples.push(await desktop.evaluate(() => {
    const g = document.querySelector('#hero-knockout g')
    const veil = document.querySelector('.hero-mark__veil')
    const m = g ? new DOMMatrixReadOnly(getComputedStyle(g).transform) : null
    return {
      scale: m && m.a ? Number(m.a.toFixed(2)) : Number(
        (g?.getAttribute('transform')?.match(/scale\(([\d.]+)\)/)?.[1] ?? 0),
      ),
...
if (!(scales[0] < 1.05)) fail(`hero zoom: starts already scaled (${scales[0]})`)
else if (!(scales[3] > scales[0] * 3)) {
  fail(`hero zoom: knockout barely grows across the exit (${scales.join(' -> ')})`)
} else if (!(scales[1] > scales[0] && scales[2] > scales[1])) {
  fail(`hero zoom: not monotonic (${scales.join(' -> ')})`)
} else console.log(`ok: knockout zooms through the exit (${scales.join(' -> ')})`)

if (!(zoomSamples[1].veil > 0.95)) {
  fail(`hero zoom: veil already dissolving at 30% — that is a crossfade, not a zoom (${zoomSamples[1].veil})`)
...
const walk = []
for (let i = 0; i <= 12; i++) {
  await desktop.evaluate((f) => window.scrollTo(0, Math.round(window.innerHeight * f * 1.02)), i / 12)
  await desktop.waitForTimeout(1100)
  walk.push(await desktop.evaluate(() => {
...
for (let i = 1; i < walk.length; i++) {
  ...
  const b = walk[i].pos
  if (!a || !b) continue
  worstStep = Math.max(worstStep, Math.hypot(b[0] - a[0], b[1] - a[1], b[2] - a[2]))
}
const crossed = new Set(walk.map((w) => w.zone))
if (!crossed.has('hero') || !crossed.has('trace')) {
  fail(`handover walk never crossed the boundary (zones: ${[...crossed].join(', ')})`)
} else if (worstStep > 0.6) {
  fail(`camera snaps at the hero/trace handover (largest step ${worstStep.toFixed(2)} units)`)
} else console.log(`ok: camera hands over to the trace without a step (max ${worstStep.toFixed(2)} units)`)
...
for (let i = 0; i < STAGES.length; i++) {
  ...
  const opened = await stageOpen(desktop, i).then(() => true).catch(() => false)
  if (!opened) {
    const dump = await desktop.evaluate((n) => ({
      zone: document.querySelector('.ui')?.getAttribute('data-scroll-zone'),
      opacity: getComputedStyle(document.querySelector(`[data-trace-beat="${n}"]`)).opacity,
    }), i)
    fail(`trace stage ${STAGES[i]}: callout never opened ${JSON.stringify(dump)}`)
    continue
  }
  ...
  const stageText = await desktop.evaluate((n) => {
    ...
    return {
      ...
      ghostOpacity: ghost ? Number(getComputedStyle(ghost.parentElement).opacity) : 0,
      headline: block?.querySelector('.trace-beat__title')?.textContent?.trim() ?? '',
    }
  }, i)
  if (stageText.ghost !== STAGES[i]) {
    fail(`trace stage ${i}: ghost word was "${stageText.ghost}", expected "${STAGES[i]}"`)
  } else if (!(stageText.ghostOpacity > 0.9)) {
    fail(`trace stage ${STAGES[i]}: ghost word never arrived (opacity ${stageText.ghostOpacity})`)
  ...
  const fx = await desktop.evaluate(() => ({
    ...
    zone: document.querySelector('.ui')?.getAttribute('data-scroll-zone'),
    heroOpacity: Number(getComputedStyle(document.querySelector('[data-testid="hero-mark"]')).opacity),
  }))
  const expected = SECTIONS.indexOf(STAGE_HOTSPOT[i])
  if (fx.zone !== 'trace') fail(`trace stage ${STAGES[i]}: zone=${fx.zone}, expected trace`)
  else if (fx.active !== expected) fail(`trace stage ${STAGES[i]}: uActive=${fx.active}, expected ${expected}`)
  else if (!(fx.focus > 0.5)) fail(`trace stage ${STAGES[i]}: uFocus=${fx.focus}, expected >0.5`)
...
for (const at of [0.05, 0.2, 0.4, 0.6, 0.8, 1]) {
  await desktop.evaluate((y) => window.scrollTo(0, y), await traceOffset(desktop, at))
  await desktop.waitForTimeout(1400)
  morphWalk.push(await desktop.evaluate(() => {
...
if (shown[0] !== 0) fail(`trace should open on the brain, got shape ${shown[0]}`)
else if (!shown.includes(1)) fail(`trace never reaches the network cloud (${shown.join(' -> ')})`)
else if (shown[shown.length - 1] !== 2) {
  fail(`trace should end on the stack cloud, got ${shown.join(' -> ')}`)
} else console.log(`ok: node field morphs brain -> network -> stack (${shown.join(' -> ')})`)

const offBrain = morphWalk.filter((s, i) => shown[i] !== 0)
if (offBrain.some((s) => s.alt < 0.9)) {
  fail(`uShapeAlt too low off the brain, connection web will show: ${JSON.stringify(offBrain)}`)
} else if (morphWalk[0].alt > 0.05) {
  fail(`uShapeAlt should be 0 on the brain, got ${morphWalk[0].alt}`)
} else console.log('ok: connection mesh gated off while the cloud is off the brain')
...
const railJump = await desktop.evaluate(async () => {
  ...
  return { count: ticks.length, before: Math.round(before), after: Math.round(window.scrollY) }
})
if (railJump.missing) fail('trace rail missing')
else if (railJump.count !== STAGES.length) fail(`trace rail: ${railJump.count} ticks, expected ${STAGES.length}`)
else if (railJump.after >= railJump.before) fail(`trace rail: clicking stage 1 did not scroll back (${railJump.before} -> ${railJump.after})`)
...
if (!(home.focus < 0.05 && home.dim < 0.05)) {
  fail(`back on hero: lobe highlight not unwound (focus=${home.focus}, dim=${home.dim})`)
} else console.log('ok: lobe highlight dissolves back on the hero')
if (!(home.heroOpacity > 0.9)) fail(`back on hero: mark did not return (opacity=${home.heroOpacity})`)
...
const spine = await desktop.evaluate((ids) => {
  ...
  for (const id of ids) {
    ...
    out.edges.push({ id, left: Math.round(r.left), right: Math.round(r.right) })
    out.titles.push({
      id,
...
} else console.log(`ok: every section container shares the same edges (${[...lefts][0]}..${[...rights][0]})`)
...
if (overhang.length) fail(`section titles overhang their column: ${JSON.stringify(overhang)}`)
else if (short.length) fail(`section titles not filling the measure: ${JSON.stringify(short)}`)
else console.log('ok: every section title fills its column measure without overhanging')

if (spine.scrollWidth > spine.clientWidth) {
  fail(`page scrolls horizontally: scrollWidth=${spine.scrollWidth} clientWidth=${spine.clientWidth}`)
...
const navJump = await desktop.evaluate(async () => {
  const btn = document.querySelector('.section-nav__item[data-section="skills"]')
  if (!btn) return { missing: true }
  btn.click()
  await new Promise((r) => setTimeout(r, 2500))
  const target = document.getElementById('section-skills').getBoundingClientRect()
  return { top: Math.round(target.top) }
})
if (navJump.missing) fail('section nav missing')
else if (Math.abs(navJump.top) > 80) fail(`section nav: skills landed at ${navJump.top}px, expected near 0`)
...
const titleReveal = await desktop.evaluate(async () => {
  ...
  return {
    ...
    animates: line.getAnimations().length > 0 || getComputedStyle(line).transitionDuration !== '0s',
...
if (titleReveal.missing) fail('section title mask missing')
  ...
  fail('section title reveal is observed on the line, which hides itself out of view')
} else if (!titleReveal.isIn) fail('section title never revealed')
else if (!titleReveal.animates) fail('section title has no transition — it would just appear')
else if (titleReveal.settled !== 'none' && titleReveal.settled !== 'matrix(1, 0, 0, 1, 0, 0)') {
...
const ghostLetters = await desktop.evaluate(() => {
  const chars = document.querySelectorAll('[data-trace-ghost="0"] .trace-ghost__ch')
  const first = chars[0]
  return {
    count: chars.length,
    staggered: chars.length > 1
      && getComputedStyle(chars[0]).transitionDelay !== getComputedStyle(chars[chars.length - 1]).transitionDelay,
    animates: first ? getComputedStyle(first).transitionDuration !== '0s' : false,
  }
})
if (ghostLetters.count < 4) fail(`ghost word not split per letter (${ghostLetters.count} spans)`)
else if (!ghostLetters.animates) fail('ghost letters have no transition')
else if (!ghostLetters.staggered) fail('ghost letters all share one delay — no stagger')
else console.log(`ok: ghost words climb in per letter (${ghostLetters.count} letters, staggered)`)
...
await portrait.waitForSelector('.ui[data-scroll-zone="hero"]', { timeout: T.settle })
await portrait.waitForTimeout(1200)

const portraitFraming = await readFraming(portrait)
if (portraitFraming.tier !== 'mobile') fail(`portrait framing: tier=${portraitFraming.tier}`)
else if (portraitFraming.camZ < 12) fail(`portrait framing: camZ=${portraitFraming.camZ}, expected >=12`)
else if (!(portraitFraming.fit > 0.04)) fail(`portrait brain fit: margin=${portraitFraming.fit}`)
else console.log(`ok: mobile portrait brain fits (margin=${portraitFraming.fit.toFixed(3)})`)
...
if (portraitMark.missing) fail('portrait: hero mark missing')
  ...
  fail(`portrait hero mark overhangs: ${portraitMark.inkLeft}..${portraitMark.inkRight} of ${portraitMark.vw}`)
} else if (portraitMark.inkTop < 0 || portraitMark.inkBottom > portraitMark.vh) {
  fail(`portrait hero mark clipped: ${portraitMark.inkTop}..${portraitMark.inkBottom} of ${portraitMark.vh}`)
...
await stageOpen(portrait, 1).catch(() => fail('portrait: trace stage 2 never opened'))
...
const spill = portraitOverflow.beats.filter((b) => b.left < -1 || b.right > portraitOverflow.vw + 1)
if (spill.length) fail(`portrait trace callouts overflow: ${JSON.stringify(spill)}`)
else if (portraitOverflow.scrollWidth > portraitOverflow.clientWidth) {
  fail(`portrait scrolls horizontally: ${portraitOverflow.scrollWidth} > ${portraitOverflow.clientWidth}`)
