...
Launch an Agent (subagent_type: `code-reviewer`, model: `inherit`) with the full rubric + all files under review. The prompt must include:
- The complete rubric
...
Launch a second Claude Agent (subagent_type: `code-reviewer`, model: `inherit`). Log a warning that both reviewers share the same model family — true model diversity was not achieved but context isolation is still enforced.
